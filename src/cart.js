...
function addToCart(cart, item, quantity, specialInstruction) {
    if (!item.isAvailable) {
    ...
    if (existing !== undefined) {
        ...
        return cart.map((c) => c.id === item.id
...
function updateCartItem(cart, itemId, newQty) {
    ...
    return cart.map((c) => c.id === itemId ? { ...c, quantity: newQty } : c);
}
...
function displayCart(cart) {
    ...
    console.log(`  ${"Subtotal".padEnd(44)} ₹${subtotal.toFixed(2).padStart(8)}`);
    console.log();
}
