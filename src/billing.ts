...
export function calculateDiscounts(
  ...
  // Membership discount
  const membershipPct = isMember(customer) ? customer.discountPercentage : 0;
  ...
  const additionalDiscount =
    subtotal > ADDITIONAL_DISCOUNT_THRESHOLD
...
export function generateBill(
  ...
  const { membershipDiscount, additionalDiscount } = calculateDiscounts(
    customer,
    subtotal
  );
...
export function displayBill(bill: SuccessBill): void {
  console.log();
  console.log("  ╔══════════════════════════════════════════════════════╗");
  ...
  // Customer info
  const memberInfo = isMember(bill.customer)
  ...
  if (bill.membershipDiscount > 0) {
    const pct = isMember(bill.customer) ? bill.customer.discountPercentage : 0;
  ...
  // Payment
  console.log();
  console.log(processPayment(bill.payment, bill.finalAmount));
